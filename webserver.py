...
def getDHTData(sensor):
	rows = db.select('SELECT timestamp, temperature, humidity FROM DHT_data WHERE sensor=:sensor ORDER BY timestamp DESC LIMIT 1', {"sensor": sensor})
	if len(rows) > 0:
		for row in rows:
			time = str(row[0])
			temperature = row[1]
...
def getHistoricalDHTData(sensor):
	rows = db.select('SELECT timestamp, temperature, humidity FROM DHT_data WHERE sensor=:sensor ORDER BY timestamp', {"sensor": sensor})
	dates = []
	temperatures = []
	humidities = []
